...
existing_files <- list.files(pattern = ".*_defender_dashboard\\.parquet$")
if (length(existing_files) > 0) {
  last_date <- max(as.Date(substr(existing_files, 1, 6), format = "%y%m%d"))
  start_date <- last_date + 1
} else {
  start_date <- as.Date("2024-11-12")
}

current_date <- Sys.Date() - 1  # Yesterday

# Only proceed if we have new dates to process
if (start_date <= current_date) {
  # Add a try-catch block for the main processing
  tryCatch({
    game_dates <- seq(start_date, current_date, by = "day")
    
    # Add logging
    message(sprintf("Processing data from %s to %s", start_date, current_date))
    
    # Create filename for today's data
    def_dash_filename <- paste0(format(current_date, "%y%m%d"), "_defender_dashboard.parquet")
    closest_def_filename <- paste0(format(current_date, "%y%m%d"), "_closest_defender_shooting_dashboard.parquet")
    ...
    combined_def_file <- "combined_defender_dashboard.parquet"
    ...
    write_parquet(def_dash, combined_def_file)
    
    # Read and combine closest defender data
    combined_closest_file <- "combined_closest_defender_dashboard.parquet"
    ...
    write_parquet(closest_defender_shooting_dash, combined_closest_file)
