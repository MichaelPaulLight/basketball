...
```{r}
start_possessions <- poss_initial %>%
  group_by(game_id, secs_passed_game, 
...
```{r}
dubs_clips <- lineup_stats |> 
  mutate(plus_minus = pts_team - pts_opp) |> 
  select(game_id, slug_team, lineup, secs_played, plus_minus, poss_team, poss_opp, garbage_time) |> 
  separate_longer_delim(lineup, delim = ", ") |> 
  rename(player = lineup) |> 
  group_by(slug_team, player) |>
  summarise(plus_minus = sum(plus_minus), 
            secs_played = sum(secs_played), 
            poss_team = sum(poss_team), 
            poss_opp = sum(poss_opp), 
            garbage_time = mean(garbage_time))

```

```{r}
zero_prop <- mean(player_stats$secs_played == 0)
...
fit_01 <- brm(
  ...
  prior = c(
    ...
    prior(exponential(2), class = "sd", group = "stint"),
